...
class Carta():
    ...
    def __gt__(self, altro):
        return self.ordineImportanza.index(self.valore) < self.ordineImportanza.index(altro.valore)
class Mazzo():
    """
...
class Tavolo():
    ...
    def Compara(self,):
        ...
        if prima.seme == seconda.seme:
            ...
            return 1
        if prima.seme == self.briscola.seme: return 0
        if seconda.seme == self.briscola.seme: return 1
        return 0
class Giocatore():
    def __init__(self):
        self.mano = Mano()
...
class GiocoBriscola():
    ...
    def __init__(self):
        pygame.init()
        self.posizioni = {'s0' : (62,282), 's1' : (182,282), 's2' : (302,282), 't0' : (122,62), 't1' : (242,62), 'b' : (422,62), 'm' : (442,62)}
        ...
        #Distribuisce carte
        self.tavolo.mazzo.DistribuisciCarte([self.listaGiocatori[0].mano, self.listaGiocatori[1].mano],6)
        #Gioca
        while 1:
            self.GiocaUnaMano()
            if self.listaGiocatori[0].mano.EVuoto(): break
        self.ContaPunti()
        if self.listaGiocatori[0].punti > self.listaGiocatori[1].punti:
            print(" "*5+"%s vince con %i punti" % (self.listaGiocatori[0],self.listaGiocatori[0].punti))
            print(" "*5+"%s totalizza %i punti" % (self.listaGiocatori[1],self.listaGiocatori[1].punti))
        elif self.listaGiocatori[1].punti > self.listaGiocatori[0].punti:
            print(" "*5+"%s vince con %i punti" % (self.listaGiocatori[1],self.listaGiocatori[1].punti))
            print(" "*5+"%s totalizza %i punti" % (self.listaGiocatori[0],self.listaGiocatori[0].punti))
        else:
            print(" "*5+"La partita finisce patta!")
        self.screen.blit(self.sfondo,(0,0))
        pygame.display.flip()
        while 1:
            if pygame.event.poll().type == QUIT: sys.exit(0)
    def GiocaUnaMano(self):
        #Scelta della carta e messa sul tavolo
        if len(self.listaGiocatori[0].mano.carte) >= 1:
            self.s0 = SpriteCarta(self.listaGiocatori[0].mano.carte[0],self.posizioni['s0'])
        else:
            self.s0 = SpriteCarta(Carta(0,11),self.posizioni['s0'])
        if len(self.listaGiocatori[0].mano.carte) >= 2: 
            self.s1 = SpriteCarta(self.listaGiocatori[0].mano.carte[1],self.posizioni['s1'])
        else:
            self.s1 = SpriteCarta(Carta(0,11),self.posizioni['s1'])
        if len(self.listaGiocatori[0].mano.carte) >= 3:
            self.s2 = SpriteCarta(self.listaGiocatori[0].mano.carte[2],self.posizioni['s2'])
        else:
        ...
        self.tavolo.vincitore = (self.tavolo.vincitore + self.cartaVincitrice) % len(self.listaGiocatori)
        print(" "*5+"%s con un %s vince la mano\n" % (self.listaGiocatori[self.tavolo.vincitore],self.tavolo.carte[self.cartaVincitrice]))
        ...
        try:
            ...
            if indiceCartaDaGiocare == 0: s0 = SpriteCarta(primaCarta, self.posizioni['s0'])
            elif indiceCartaDaGiocare == 1: s1 = SpriteCarta(primaCarta, self.posizioni['s1'])
            elif indiceCartaDaGiocare == 2: s2 = SpriteCarta(primaCarta, self.posizioni['s2'])
        except IndexError:
            pass
            ...
            for event in pygame.event.get():
                ...
                if event.type == MOUSEBUTTONUP:
                    ...
                    for s,j in [(self.s0, 0), (self.s1, 1), (self.s2, 2)]:
                        if self.t1.rect.colliderect(s) : return j
                    for s,j in [(self.s0, '0'), (self.s1, '1'), (self.s2, '2')]:
                        s.rect.topleft = self.posizioni['s'+j]
                if event.type == MOUSEBUTTONDOWN and event.button == 1 and self.s0.rect.collidepoint(event.pos):
                    self.down = True
                    pressed = 's0'
                if event.type == MOUSEBUTTONDOWN and event.button == 1 and self.s1.rect.collidepoint(event.pos):
                    self.down = True
                    pressed = 's1'
                if event.type == MOUSEBUTTONDOWN and event.button == 1 and self.s2.rect.collidepoint(event.pos):
                    self.down = True
                    pressed = 's2'
                if self.down == True and event.type == MOUSEMOTION and pressed == 's0':
                    self.s0.update(event.rel)
                if self.down == True and event.type == MOUSEMOTION and pressed == 's1':
                    self.s1.update(event.rel)
                if self.down == True and event.type == MOUSEMOTION and pressed == 's2':
                    self.s2.update(event.rel)
                pygame.display.flip()
            self.clock.tick(400)
    def draw(self):
        self.screen.blit(self.sfondo,(0,0))
        if self.pressed != 's0': self.s0.draw(self.screen)
        if self.pressed != 's1': self.s1.draw(self.screen)
        if self.pressed != 's2': self.s2.draw(self.screen)
        if self.tavolo.vincitore == 1: self.t0.draw(self.screen)
        ...
        for indiceGiocatore in range(len(self.listaGiocatori)):
            for carta in self.listaGiocatori[indiceGiocatore].prese:
                if carta.valore in listaPunteggi.keys():
                    self.listaGiocatori[indiceGiocatore].punti+=listaPunteggi[carta.valore]    
